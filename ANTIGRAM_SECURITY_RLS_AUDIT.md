...
2026-07-06: частично исправлено на клиенте. Массовые публичные join/list-запросы в `src/lib/db.ts` и `src/lib/support.ts` переведены на явный public-safe набор полей:

```text
id, username, display_name, bio, avatar_url, website, created_at
```

Остаточный риск: `getProfile(userId)` всё ещё использует `select('*')`, потому что сейчас он одновременно используется для текущего пользователя в `AuthContext` и для чужого профиля. Следующий hardening: разделить `getOwnProfile()` и `getPublicProfile()`.
...
2. Разделить `getOwnProfile()` и `getPublicProfile()`, чтобы чужие профили никогда не грузились через `select('*')`.
